...
\Statex
\State $V_{\text{desc}} \gets \Call{GetAllDescendants}{G, v_f}$
\State $\Call{PruneSubgraph}{G, V_{\text{desc}}}$
\Statex
\State $\mathbf{E}_{\text{err}} \gets \Call{ExtractErrorSignature}{v_f}$
\State $\mathbf{C}_{\text{aug}} \gets \Call{AppendContext}{\mathbf{C}, \mathbf{E}_{\text{err}}}$
\Statex
\State $V_{\text{alt}} \gets \Call{SynthesizeAlternativeNodes}{\mathbf{C}_{\text{aug}}, \theta_{\min}}$
\Statex
\For{each $n_{\text{alt}} \in V_{\text{alt}}$}
    \State $G.\Call{InjectNode}{n_{\text{alt}}}$
    \State $\Call{ReconnectEdges}{G, n_{\text{alt}}, G.\Call{GetSinks}{v_f}}$
\EndFor
\Statex
