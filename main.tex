...
\author{
    \IEEEauthorblockN{Fernando M. F.$\dagger$}
...
\begin{tabular}{lcccc}
\toprule
\textbf{System} & \textbf{Routing} & \textbf{Memory} & \textbf{Fault Tolerance} & \textbf{Infrastructure Awareness}\\
\midrule
LangChain~\cite{langchain2023} & Static / Hardcoded & Conversational Buffer & Limited (Retries) & No\\
\addlinespace
CrewAI~\cite{crewai2023} & Role-based Heuristics & Session-based Cache & Minimal & No\\
\addlinespace
AutoGen~\cite{autogen2024} & Conversational Loops & Context-Window Append & Partial (Agent Loops) & No\\
\addlinespace
Semantic Kernel~\cite{semantic_kernel2024} & Embedding Matching & Linear Plugin Memory & Limited & Partial (Static)\\
\addlinespace
\textbf{XING (Ours)} & \textbf{Adaptive Optimization} & \textbf{Semantic Graph Fabric} & \textbf{Self-Healing DAGs} & \textbf{Native (Cost/Lat/Energy)}\\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{figure}[t]
    ...
    \caption{Five-layer hierarchical architecture with telemetry feedback loops between the routing engine and distributed infrastructure.}
